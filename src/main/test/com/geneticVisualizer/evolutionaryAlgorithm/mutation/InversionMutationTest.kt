...
internal class InversionMutationTest {

    var initializer = QuadrantInitial(-465.0,465.5,-335.0,335.0)
    var population= initializer.initialize(BinaryGenome(), 500)

    var inversionMutation: InversionMutation = InversionMutation()
    @Test
    fun mutation() {
        var childPopulation1 = inversionMutation.mutation(population,100.0)
        assertEquals(500,childPopulation1.size)
        var childPopulation2 = inversionMutation.mutation(population,0.0)
