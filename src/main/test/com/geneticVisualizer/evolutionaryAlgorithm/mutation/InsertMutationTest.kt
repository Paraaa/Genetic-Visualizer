...
internal class InsertMutationTest {
    var initializer = QuadrantInitial(-465.0,465.5,-335.0,335.0)
    var population= initializer.initialize(VectorGenome(), 500)

    var insertMutation: InsertMutation = InsertMutation()
    @Test
    fun mutation() {
        var childPopulation1 = insertMutation.mutation(population,100.0)
        assertEquals(500,childPopulation1.size)
        var childPopulation2 = insertMutation.mutation(population,0.0)
